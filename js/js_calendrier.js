...
if(note[jj+"-"+mm]!=null) {
	if (jourPause[xx]||jourFeriee[jj+"-"+mm]!=null)
		styleX+="background:#81C49E;";
	else
		styleX+="background:#B5FED5;";
}
else {
if (jourPause[xx]||jourFeriee[jj+"-"+mm]!=null)
...
function view_microcal(actif,ki,source,mxS,axS)
{
	if (actif)
	{
		...
		htm+="<td style='cursor:hand;'><a href=\"javascript:view_microcal(true,'"+ki+"','"+source+"',"+(dateS.mois-1)+","+dateS.annee+");\">-</a></td>";
		htm+="<td colspan='5'> <b> "+moisX[dateS.mois]+"</b>&nbsp;"+dateS.annee+"</td>";
		htm+="<td style='cursor:hand;'><a href=\"javascript:view_microcal(true,'"+ki+"','"+source+"',"+(dateS.mois+1)+","+dateS.annee+")\">+</a></td></tr>";
		//--corps
		htm+="<tr>";
		...
		for (y=0;y<=ymaxi;y++)
		{
			...
			for (x=0;x<7;x++)
			{
				...
				if (idxP<=0)
				{
					...
					if (mm==0)
					{mm=12;aa--;}
					htm+="<td style='"+testTravail(false,xx,jj,mm,aa)+"' onmouseover='choix(this,true)' onmouseout='choix(this,false)' onclick=\""+ki+".value='"+((jj<10)?"0":"")+jj+"/"+((mm<10)?"0":"")+mm+"/"+aa+"';"+ki+".style.color='black';\"><a href='Calendrier-" + jj + "_" + mm + "_" + aa + "'>"+jj+"</a></td>";
				}
				else if (idxP>jmaxi) //jour du mois suivant
				{
					...
					htm+="<td style='"+testTravail(false,xx,jj,mm,aa)+"' onmouseover='choix(this,true)' onmouseout='choix(this,false)' onclick=\"document.getElementById('"+ki+"').value='"+((jj<10)?"0":"")+jj+"/"+((mm<10)?"0":"")+mm+"/"+aa+"';document.getElementById('"+ki+"').style.color='black';\"><a href='Calendrier-" + jj + "_" + mm + "_" + aa + "'>"+jj+"</a></td>";
				}
				else //jour du mois en cours
				{
					jj=idxP;mm=dateS.mois;
					htm+="<td style='"+testTravail(true,xx,jj,mm,aa)+"' onmouseover='choix(this,true)' onmouseout='choix(this,false)' onclick=\"document.getElementById('"+ki+"').value='"+((jj<10)?"0":"")+jj+"/"+((mm<10)?"0":"")+mm+"/"+aa+"';document.getElementById('"+ki+"').style.color='black';\"><a href='Calendrier-" + jj + "_" + mm + "_" + aa + "' class='noir' >"+jj+"</a></td>";
				}
			}
			htm+="</tr>"
		}//-------------------------
